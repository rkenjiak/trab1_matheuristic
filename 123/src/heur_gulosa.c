...
int gulosa(SCIP* scip, SCIP_SOL** sol, SCIP_HEUR* heur)
{
   ...
   vetor = (tvet *) malloc(n*sizeof(tvet));
   nInSolution = 0;
   nCovered = 0;
   nCands = 0;
   custo = 0;
      ...
      if(SCIPvarGetLbLocal(var) > 1.0 - EPSILON){ // var >= 1.0
        if (i<n){
          ...
          for(j=0;j<I->item[i].nsets;j++){
             ...
             if(forfeit[ii] > I->S[ii].h){ //atualizar violations?
               violations+=1;
             }
          }
   ...
   tam = n;
   for(i=0;i<tam;i++){
      vetor[i].index = I->item[i].label;
      vetor[i].densidade = ((float)I->item[i].value)/((float)I->item[i].weight);
   }
   qsort(vetor, tam, sizeof(tvet), cmpfunc);
